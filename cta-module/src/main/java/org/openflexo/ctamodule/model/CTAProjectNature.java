...
/**
 * This API is used to interpret a {@link FlexoProject} as a CTA project <br>
 *
 * 
 * @author yourname
 */
@ModelEntity
@XMLElement
@ImplementationClass(CTAProjectNature.CTAProjectNatureImpl.class)
public interface CTAProjectNature extends ProjectNature<CTAProjectNature> {

	@PropertyIdentifier(type = CTAInstance.class)
	public static final String CTA_INSTANCE = "CTAInstance";

	@Getter(value = CTA_INSTANCE, inverse = CTAInstance.NATURE)
	@XMLElement
	public CTAInstance getCTAInstance();

	@Setter(CTA_INSTANCE)
	public void setCTAInstance(CTAInstance ctaInstance);

	public VirtualModel getCTAViewPoint();

	public abstract class CTAProjectNatureImpl extends ProjectNatureImpl<CTAProjectNature> implements CTAProjectNature {
		@SuppressWarnings("unused")
		private static final Logger logger = Logger.getLogger(CTAProjectNature.class.getPackage().getName());

		private VirtualModel ctaViewpoint;

		@Override
		public VirtualModel getCTAViewPoint() {
			if (ctaViewpoint == null && getServiceManager() != null) {
				try {
					ctaViewpoint = getServiceManager().getVirtualModelLibrary().getVirtualModel(CTACst.CTA_VIEWPOINT_URI);
