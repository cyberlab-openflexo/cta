...
public class PimCADiagramModuleView extends FMLControlledDiagramModuleView {

	@SuppressWarnings("unused")
	private static final Logger logger = Logger.getLogger(PimCADiagramModuleView.class.getPackage().getName());

	private DGListener dgListener;
	private DSListener dsListener;

	public PimCADiagramModuleView(FMLControlledDiagramEditor editor, PimCAPerspective perspective) {
		super(editor, perspective);
		dgListener = new DGListener(perspective.getPimCAModelBrowser().getFIBBrowserWidget().getTechnologyComponent().getJTree());
		dsListener = new DSListener();
	}

	@Override
	public PimCAPerspective getPerspective() {
		return (PimCAPerspective) super.getPerspective();
	}

	@Override
	public String getInfoMessage() {
		return "PimCA modelling - CTRL-drag to draw edges";
	}
	...
	public void willHide() {
		...
		getPerspective().hidePimCAModelBrowser();
	}
	...
	public void willShow() {
		...
		FMLRTVirtualModelInstance pimCAModel;
		try {
			pimCAModel = fmlControlledDiagram.execute("model");
			getPerspective().showPimCAModelBrowser(pimCAModel);
			// Activate drag&drop between browser and diagram
			getPerspective().getPimCAModelBrowser().getFIBBrowserWidget().registerDragGestureListener(dgListener);
		...
		public void dragGestureRecognized(DragGestureEvent e) {
			...
			if (dragNode != null) {
				...
				try {
					...
					logger.fine("Starting drag for " + dragNode.getRepresentedObject());
